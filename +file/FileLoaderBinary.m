classdef FileLoaderBinary < file.FileLoaderMethod
    ...
    methods
        ...
        function starttime=getRecordStartTime(obj)
            try
                S = utilities.xml2struct.xml2struct(obj.xmlfile);
                starttime=datetime(S.SETTINGS.INFO.DATE.Text , ...
                    'InputFormat','dd MMM yyyy HH:mm:ss');

                ps=S.SETTINGS.SIGNALCHAIN.PROCESSOR;
                for ipro=1:numel(ps)
                    p=ps{ipro};
                    if ismember('PhoStartTimestampPlugin', ...
                            fieldnames(ps{ipro}))
                        disp('Using PhoStartTimestampPlugin')
                        starttime1=...
p.PhoStartTimestampPlugin.RecordingStartTimestamp.Attributes.startTime;
                        ...
                        %%%%%% DELETE LATER %%%%%% 
                        tmp = split(obj.OEBinFile,'/');
                        if ismember('James',tmp) 
                        disp('Subtracting 4 hrs from photimestamp')
                        disp(['Original photimestamp is ' char(starttime2)])
                        starttime2 =starttime2 - hours(4);
                        disp(['Adjusted photimestamp is ' char(starttime2)])
                        end
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function openEphysRecord = load(obj)
            ...
            if datFileSize<=obj.getDatFile.bytes
                ...
                D= openEphys.load_open_ephys_binary(obj.OEBinFile,'continuous',1, ...
                    'mmap','.dat');toc
            ...
            rec=C{end};cs=strsplit(rec,'recording');recno=str2double(cs{2}); %get recording number
            if recno==1
            ...
            try
                openEphysRecord.evts= openEphys.load_open_ephys_binary( ...
                    obj.OEBinFile,'events',1,'mmap','.dat');toc
            catch
                log.warning('No event data.')
            end
            try
                openEphysRecord.spks= openEphys.load_open_ephys_binary( ...
