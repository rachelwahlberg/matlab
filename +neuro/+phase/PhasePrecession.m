classdef PhasePrecession < neuro.phase.SpikePhases
    % Get initial phases from neuro.spike.SpikeUnitLFP. 
    ...
    methods
        function obj = PhasePrecession(spikePhases,position,time)
        ...
        function [] = plotPrecession(obj,color)
            % Plot theta phase against position
...
%             [~,c]=contour(X,Y,N1',5,LineWidth=1);
        ...
        function [] = plotPrecessionTime(obj)
            %Plot theta phase against spiketime
...
%             colormap(ax,colorsBlue(3:end,:));
%             [~,c]=contour(X,Y,N1',5,LineWidth=1);
            idx=isnan(t)|isnan(ph1);
            t(idx)=[];
            ph1(idx)=[];
            obj.Position(idx,:)=[];
            obj.PolarData(idx,:)=[];
            a=fit(t,ph1,'poly1');
            x1=linspace(min(t),max(t),20);
            try
                [rho, pval] = obj.getTestCorrcl(t);
            catch ME
                
            end
            if pval<.05
                str=sprintf('Corr Time x Phase');
                text(5,6,str,Units="characters",Color=colorsMatlab(3,:));
                plot(x1,a.p1*x1+a.p2,"Color",colorsMatlab(1,:),LineWidth=2);

            else
                str=sprintf('No Corr Time x Phase');
                text(5,6,str,Units="characters",Color=colorsMatlab(2,:));                
                plot(x1,a.p1*x1+a.p2,"Color",colorsMatlab(1,:),LineWidth=1);
            end
            text(5,5,sprintf('rho=%.3f, p=%.3f',rho,pval),Units="characters");
