function  PlaceMaps = pc_clockcounter_linear(mf,plotfigures)
%%%%% uses PFClassic: %%%%%%%
...
smoothVal = .04; %out of 1
nGrid = [100 100]; %one val for each dimension + for time
timebinsize = .01;% tbins in units of seconds, e.g. 10 ms.

%%%%%%%%% time %%%%%%%%%%%%%%%%
positionTimeZT = mf.positionData.time.getTimePointsZT;

%%%%%%%% Normalize position  %%%%%%%%%%%%%%
x = mf.positionData.data.X;
z = mf.positionData.data.Z;
...
%%%% get angle
[Turns,angle] = directionsCircularTrackRadians(mf.positionData);
at = [angle xzt(:,3)];
...
sa = sf.getPhyOutputFolder(mf.phypath); %Spike Array (Kaya code)
...
for c = 1%:nclassTypes
    ...
    for unit = 1:nUnits %nUnits
        ...
        spksBinned = histcounts(spksZT,histedges)'; %still applies to linear
        ...
        [~,spksBinnedcl,spksBinnedco] = counterClockwise_positionBinned(positionBinned,mf.Events.Turns,spksBinned);
        ...
        plotplacemap = 0;
        [placemapAll, spikemapAll, timeSpentAll] = neuro.placeField.PFClassic1D_RW(allAngles,spksBinned,smoothVal,100,timebinsize,plotplacemap);
        peakAll = neuro.placeField.getPlacefieldPeakCircular(placemapAll);

        [placemapClock, spikemapClock, timeSpentClock] = neuro.placeField.PFClassic1D_RW(allAngles,spksBinnedcl,smoothVal,100,timebinsize,plotplacemap);
        peakClock = neuro.placeField.getPlacefieldPeakCircular(placemapClock);

        [placemapCounter, spikemapCounter, timeSpentCounter] = neuro.placeField.PFClassic1D_RW(allAngles,spksBinnedco,smoothVal,100,timebinsize,plotplacemap);
        peakCounter = neuro.placeField.getPlacefieldPeakCircular(placemapCounter);

        if unit == 1 || unit == 3
          
            PlaceMaps = cell([]);
            PlaceMaps.peakAll(unit,:) = peakAll;
            PlaceMaps.peakClock(unit,:) = peakClock;
            PlaceMaps.peakCounter(unit,:) = peakCounter;
        else
            PlaceMaps.peakAll = vertcat(PlaceMaps.peakAll,peakAll);
            PlaceMaps.peakClock = vertcat(PlaceMaps.peakClock,peakClock);
            PlaceMaps.peakCounter = vertcat(PlaceMaps.peakCounter,peakCounter);
        end

        PlaceMaps.placeMapAll(unit,:) = placemapAll;
        PlaceMaps.placeMapClock(unit,:) = placemapClock;
        PlaceMaps.placeMapCounter(unit,:) = placemapCounter;
        PlaceMaps.spikeMapAll(unit,:) = spikemapAll;
        PlaceMaps.spikeMapClock(unit,:) = spikemapClock;
        PlaceMaps.spikeMapCounter(unit,:) = spikemapCounter;
        PlaceMaps.timeSpentAll(unit,:) = timeSpentAll;
        PlaceMaps.timeSpentClock(unit,:) = timeSpentClock;
        PlaceMaps.timeSpentCounter(unit,:) = timeSpentCounter;
...
for unit = 1:63
tmpTimeSpent(unit,:) = timeSpentAll;
end
    ...
    function plotFigures
       ...
       % x = x*100; z = z*100;
        ...
        plot.plotPlaceFields('spkLocations',plottitle,x*100,z*100,xpostvelocityfilter,zpostvelocityfilter)
        ...
        plot.plotPlaceFields('spkLocations',plottitle,x*100,z*100,xclockwise,zclockwise)
        ...
        plot.plotPlaceFields('spkLocations',plottitle,x*100,z*100,xcounter,zcounter)
        ...
        % plot linear on separate figure cause of zoom factor
        plotplacemap = 1;
        figure 
        h2 = tiledlayout(1,3);
        position = 1; h = 1; w = 1; axb1 = nexttile(position,[h,w]);
        position = 2; h = 1; w = 1; axb2 = nexttile(position,[h,w]);
        position = 3; h = 1; w = 1; axb3 = nexttile(position,[h,w]);

        axes(axb1)
        external.Placefields.PFClassic1D(allAngles,spksBinned,smoothVal,100,timebinsize,plotplacemap);
        title('Linear place field - all')

        axes(axb2)
        external.Placefields.PFClassic1D(allAngles,spksBinnedcl,smoothVal,100,timebinsize,plotplacemap);
        title('Linear place field - clockwise')

        axes(axb3)
        external.Placefields.PFClassic1D(allAngles,spksBinnedco,smoothVal,100,timebinsize,plotplacemap);
        title('Linear place field - counter')
