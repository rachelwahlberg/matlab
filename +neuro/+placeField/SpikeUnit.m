classdef SpikeUnit
    ...
    %%%% FUNCTIONS BELOW %%%%%%%%%%%%%%%%%%%%
%     obj = SpikeUnit(spikeId,spikeTimes,timeIntervalCombined)
%     timesnew = getAbsoluteSpikeTimes(obj)
%     times = getTimes(obj)
%     obj = setInfo(obj,info)
%     fireRate = getFireRate(obj,timebininsec)
%     [] =plotOnTimeTrack(obj,track,speedthreshold)
%     [] = plotOnTrack(obj,track,speedthreshold)
%     [] = plotPlaceFieldBoth(obj,track,speedthreshold)
%     [] = plotPlaceFieldNeg(obj,track,speedthreshold)
%     [] = plotPlaceFieldPos(obj,track,speedthreshold)
%     info=getInfo(obj,idx)
%     str=addInfo(obj,idx)
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    methods
        ...
        function timesnew = getAbsoluteSpikeTimes(obj)
            ...
            t = obj.Times;
            timesnew=ticd.getRealTimeFor(double(t));
        end
        ...
        function [] = plotPlaceFieldBoth(obj,positionData,speedthreshold) %5 cm/sec is a standard thresh
            %METHOD1 Summary of this method goes here
            %   Detailed explanation goes here
            times=obj.getAbsoluteSpikeTimes;
            [~,~,~,idx]=positionData.getLocationForTimesBoth(times,speedthreshold);
            idx_track=positionData.getSpeed.getAboveAbs(speedthreshold);
            SpkCnt=positionData.getSpikeArray(times(idx));
            Pos1=[positionData.Z positionData.X];
            Pos = (Pos1 - min(Pos1)) ./ ( max(Pos1) - min(Pos1) );
            SpatialBinSizeCM=2;
            nGrid(1)=round((max(positionData.Z(idx_track))-min(positionData.Z(idx_track)))/SpatialBinSizeCM);
            nGrid(2)=round((max(positionData.X(idx_track))-min(positionData.X(idx_track)))/SpatialBinSizeCM);
            Smooth=.075;
            Tbin=1/30;
            TopRate=[];
