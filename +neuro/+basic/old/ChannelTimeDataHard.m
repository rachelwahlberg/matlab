classdef ChannelTimeDataHard < file.BinarySave
    ...
    %%%%% FUNCTIONS BELOW %%%%%%%%%%%%%%
%    newObj = ChannelTimeDataHard(filepath)
%     str=toString(obj)
%     chnames=getChannelNames(obj)
%     ctdh=getChanneltimeDataHardBad(obj)
%     ch = getChannel(obj,channelNumber)
%     obj = addChannel(obj,channels)
%     filewrite = addPositionFile(obj, channels)
%     obj = removeChannel(obj,channel)
%     LFP = getChannelsLFP(obj,channelNumber)
%     ctd = get(obj,channels,time)
%     newobj = getDownSampled(obj, newRate, newFileName)
%     out=getFolder(obj)
%     out=getFilepath(obj)
%     out=getData(obj)
%     out=getTimeIntervalCombined(obj)
%     out=getProbe(obj)
%     obj=setTimeIntervalCombined(obj,ticd)
%     obj=setProbe(obj,probe)
    ...
    methods
        function newObj = ChannelTimeDataHard(filepath)
            if isa(filepath,'neuro.basic.ChannelTimeDataHard')
                ...
                if ~exist('filepath','var')
                    ...
                    defpath='/data/ExperimentsRSW/CircularMaze/';
                ...
                try
                    newObj.TimeIntervalCombined=time.TimeIntervalCombined(folder);
                catch
                    ...
                    ti=time.TimeInterval(startTime, sampleRate, numberOfPoints);
        ...
        function filewrite = addPositionFile(obj, channels)
            ...
            for i = 1:length(array)
                if array(1,i)==306 && array(2,i)==297
                    array(:,i)=[0,0];
                end
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
