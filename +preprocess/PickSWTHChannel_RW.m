function [SleepScoreLFP,PickChannelStats] = PickSWTHChannel_RW(basePath,recordingname,scoretime,SWWeightsName,...
...
addParameter(p,'downsamplefactor',1);% was 5 before
...
rawlfpfile = fullfile(basePath,recordingname);
if exist(rawlfpfile)
[datasetfolder,basename,extension] = fileparts(rawlfpfile);
else
    error('No LFP file')
end
%basename = [basename extension];

matfilename = fullfile(basePath,[basename,'.SleepScoreLFP.LFP.mat']);
...
xmlfilename = fullfile(datasetfolder,[basename '.xml']);
...
allLFP = bz_GetLFP(usechannels,'basepath',basePath,'basename',basename,...
...
%             'min.  ETR: ',num2str(round(estimatedremaining./60)),'min.'])
  % end
    ...
    testLFP = allLFP.data;
    test_ts = allLFP.timestamps;
    for i = 1:size(ignoretime,1)
        testLFP(ignoretime(i,1):ignoretime(i,2),:) = nan;
        test_ts(ignoretime(i,1):ignoretime(i,2),:) = nan;
    end
...
%         ignoretimeIDX = InIntervals_RW(t_FFT,ignoretime);
...
swthLFP = bz_GetLFP([SWchanID,THchanID],'basepath',basePath,'basename',basename,...
...
subplot(5,1,3)
 %   plot(allLFP(:,1),allLFP(:,goodSWidx),'k')
        ...
        if saveFiles
saveas(chanfig,[figfolder,basename,'_SWTHChannels'],'jpeg')
        end
