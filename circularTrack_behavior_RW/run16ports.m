...
%rat = 'test';
rat = 'luna_watertraining_jan30_2';
%rat = 'test';
...
pumpOpen = .04; % in seconds, length of time water is delivered (small because of the gravity system)
%don't go lower than 0.02! solenoid won't work. sometimes unreliable up to
%0.7 as well.

%bInside_pcb = [12 13 14 15];
bInside_pcb = [3 13 14 15]; % JUST TMP ON 6/30 CAUSE OF CHEWED CABLE
...
%% prefill wells
testwells(valves_pcb(1),pumpOpen);
testwells(valves_pcb(2),pumpOpen);
