function [] = run16ports_manualSwitch()

% for MANUAL water delivery. See run16ports_autoSwitch for auto switching using sensors.
 % 
%  testwells(0,0.5)
% testwells(1,0.5)
%  testwells(2,0.5)
...
sensors_pcb{1} = [3 1]; % 12 14
valves_pcb{1} = [1 2];
sensors_pcb{2} = [3 1]; %1 3
valves_pcb{2} = [1 2];

pumpOpen = 0.06; % .04; % in seconds, length of time water is delivered (small because of the gravity system)
...
%%%%%% port visits %%%%%%
fnamePorts = [session_basepath '\National_Instruments\portTimestamps_' rat '_' region '_' currentdate '_session' session_number '.txt'];
fIDd = fopen(fnamePorts,'a');

%%%%% start/transition times %%%%%
fnameTransition = [session_basepath '\National_Instruments\portTransitions_' rat '_' region '_' currentdate '_session' session_number '.txt'];
...
for pair = 1:2
    ...
    if pair == 2 % draw a transition line
        ...
        xline(h,minutes(start2-starttime))
       ...
       if activeport == 0
        ...
        if repeat == 0 || seconds(d-newsensortime)>=15
           ...
           %scatter(h,d,1)
            ...
            fprintf(fIDd,' %s %d \n', string(d,'HH:mm:ss:SSS'), activeport-1); %then write to file. -1 to get back to 0 indexing
...
function keyboardcontrol(src,event)
%MATLAB CODING
