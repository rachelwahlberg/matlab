classdef (Abstract) FigurePipeline < handle
    ...
    methods
        ...
        function initializeProperties(obj)
            ...
            obj.SpikeTable = sa.SpikeTable;
            obj.ClusterInfo = sa.ClusterInfo;
            obj.Info = obj.Info;

            obj.positionData = loadPositionData(obj);

            %%%%%%%%%%%%%% timestamps %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            theFile=dir(fullfile(obj.phypath,['*1250Hz.TimeIntervalCombined*' '.csv'])); %TIC file gives timing info for each pre-merge file
            if isempty(theFile)
                logger.warning(strcat('TimeIntervalCombined is not loaded. \n\tLocation:\t',foldername,'\n'));
            end

            ticd=neuro.time.TimeIntervalCombined(fullfile(theFile.folder, theFile.name)); % from the neuro.time folder
        ...
        function positionData = loadPositionData(obj)
            ...
            for f = 1:length(fnames)
                filenames{f} = fullfile(obj.optipath,fnames(f).name);
            end

            ot = optiTrack.OptiLoader.instance();
            op = ot.loadFile(filenames);
            positionData = op.getMergedPositionData;
        end

        function interpolatedData = interpPositionData(obj)
