                ...
                if isstring(el)||ischar(el)
                    ...
                    try
                        ...
                        for iti=1:height(T)
                            ...
                            if exist('zt','var')
                                theTimeInterval=time.TimeIntervalZT(theTimeInterval,zt);
                            end
...
%             end
        ...
        function obj = plus(obj,varargin)
            ...
            for iArgIn=1:nargin-1
                ...
                if ~isempty(theTimeInterval)
                    try
                        assert(isa(theTimeInterval,'time.TimeInterval'));
                        obj.timeIntervalList.add(theTimeInterval);
                        l=logging.Logger.getLogger;
                        l.fine(sprintf('\nRecord addded:\n%s',theTimeInterval.tostring));
                    catch
                        assert(isa(theTimeInterval,'time.TimeIntervalCombined'));
        ...
        function ti=mergeTimeIntervals(obj)
            ti=time.TimeInterval(obj.getStartTime, obj.getSampleRate, obj.getNumberOfPoints);
        end
        ...
        function ticd=readTimeIntervalTable(~,table)
            ...
            for iti=1:height(T)
                tiRow=T(iti,:);
                ti=time.TimeInterval(tiRow.StartTime,tiRow.SampleRate,tiRow.NumberOfPoints);
                ticd=ticd+ti;
            end
        end
