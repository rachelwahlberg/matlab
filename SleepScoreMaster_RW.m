function SleepState = SleepScoreMaster_RW(basepath,varargin)
...
elseif ~isempty(dir([basepath '*.eeg']))
    ...
    error(['no .lfp file in basePath, pick a selection of session folders',...
...
[datasetfolder,basename,extension] = fileparts([basepath recordingname]);
...
% savefolder = fullfile(savedir,recordingname); %recordingname being a .lfp filename?
% if ~exist(savefolder,'dir')
%     mkdir(savefolder)
% end
%savefolder = fullfile(datsetfolder,basename);

%Filenames of metadata and SleepState.states.mat file to save
sessionmetadatapath = fullfile(datasetfolder,[basename,'.SessionMetadata.mat']);
%Buzcode outputs
bz_sleepstatepath = fullfile(datasetfolder,[basename,'.SleepState.states.mat']);
