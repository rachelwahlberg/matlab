...
for a = 1:length(sessions)
  ...
  %      for fnum = 1:length(animalStructures{a}.preTables)
        ...
        percentblack(fnum,a) = sum(ismember(ports,b_insideports))/sum(ismember(ports,[b_insideports b_outsideports]));
...
lunaD = [percentwhite(:,1) percentblack(:,1)];
minervaD = [percentwhite(:,2) percentblack(:,2)];
...
hold on
title('Rat L')
v1 = violin(lunaD,'facecolor',[1 1 1;0 0 0],'facealpha',1);
scatter(ones(length(lunaD),1),lunaD(:,1),'m','filled')
scatter(ones(length(lunaD),1)*2,lunaD(:,2),'m','filled')
legend({' ' 'Mean' 'Median' }) %imperfect fix
%legend("off")

ylabel('Discrimination')
ax1.XTickLabel = [];
xlabel('white side          black side')

axes(ax2)
hold on
title('Rat M')
v1 = violin(minervaD,'facecolor',[1 1 1;0 0 0],'facealpha',1);
scatter(ones(length(minervaD),1),minervaD(:,1),'m','filled')
scatter(ones(length(minervaD),1)*2,minervaD(:,2),'m','filled')
legend('off')
%legend({' ' 'Mean' 'Median' }) %imperfect fix
%ylabel('Time to twenty trials (min)')
ax2.XTickLabel = [];
