...
% for MANUAL water delivery. 
% see run16ports_autoSwitch for auto switching using sensors.
...
fnamePorts = ['E:\Rachel\Datapoints\datapoints_manual_' num2str(yyyymmdd(datetime)) '_'  t '_' rat '.txt'];
    ...
    while true && switchpairs == 0
      ...
      %  port =di.readNI; % default is zero if it's not being triggered, one if it is.
       ...
       % if isempty(activeport)
        ...
        % get activeport  from the spacebar
...
elseif strcmp(event.Key,'o') % visited port 8
    assignin("caller","activeport",8)
elseif strcmp(event.Key,'p') % visited port 9
    assignin("caller","activeport",9)
elseif strcmp(event.Key,'a') % visited port 10
   assignin("caller","activeport",10)
elseif strcmp(event.Key,'s') % visited port 11
    assignin("caller","activeport",11)
elseif strcmp(event.Key,'d') % visited port 12
  assignin("caller","activeport",12)
elseif strcmp(event.Key,'f') % visited port 13
    assignin("caller","activeport",13)
elseif strcmp(event.Key,'g') % visited port 14
    assignin("caller","activeport",14)
elseif strcmp(event.Key,'h') % visited port 15
